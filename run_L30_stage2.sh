...
run_chains(){ # TAG SEEDS STARTS ADCOV FLAGS
  ...
  for SEED in $SEEDS; do
    ## ⚠ NOT `tr ... | grep -q`: under `set -o pipefail` a matching `grep -q` exits early, `tr` takes SIGPIPE,
    ## and the PIPELINE reports failure — so a check that PASSED prints its alarm (seeds 2027/2029, 09-22, the
    ## alarm was false; both banners were in both logs). Count instead: grep reads to EOF, nothing is signalled.
    [[ $(tr '\r' '\n' < outputs/mcmc/log_${T}_seed${SEED}.txt | grep -ac "L30 flag: ais-ramp=true") -gt 0 ]] || say "  *** seed$SEED: ramp NOT announced in its log ***"
    [[ $(tr '\r' '\n' < outputs/mcmc/log_${T}_seed${SEED}.txt | grep -ac "fresh diagonal for .*ais_ramp_gon") -gt 0 ]] || say "  *** seed$SEED: ramp rows not fresh in the proposal ***"
  done
}
