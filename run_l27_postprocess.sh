#!/bin/bash
## L27 POSTPROCESS (2026-09-20): waits for the four L27 chains, then the postprocess chain of steps. CONTROL = L26 (one axis: the A/C/D reduction).
...
T=L27
CTRL=L26
LOG=outputs/log_l27_postprocess_driver.txt
...
say "  EXPECT paleo-priors=true no-delta=true no-d2-gsic=true obs-corr-len=100.0 precip-reparam=true + cut-fastdyn fix-gamma no-ledger; 50 free params. If not — STOP."
