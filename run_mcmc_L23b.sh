#!/bin/bash
## L23b — RECONSTRUCTED 2026-09-02. L23's RNG-ONLY replicate — seeds 3026-3029 map onto the SAME over-dispersed start rows (a6fd634), so it isolates sampler noise. Ran 2026-09-01 09:20. It is the NOISE FLOOR for every between-vintage claim.
##
## ⚠⚠ THIS SCRIPT DID NOT EXIST WHEN L23b RAN, AND THAT IS WHY IT EXISTS NOW. Three flags were
...
##   * --adcov  : from the run's own outputs/mcmc/seed_diag_L23b_seed*.txt
...
## Pinning means this script reproduces L23b whenever it is run. The original command would
...
##
##   bash run_mcmc_L23b.sh [n_iter]        # default 2000000
set -e
cd "$(dirname "$0")"
NITER="${1:-2000000}"
TAG="${TAG:-L23b}"
...
echo "$TAG: 4 chains x $NITER (seeds 3026 3027 3028 3029) — amp ~ N(1.09, 0.10), adcov=$ADCOV"
for SEED in 3026 3027 3028 3029; do
