...
> MAGICC-SLR runs to **2300** (the comparison script's docstring saying it ends at 2100 is
> **stale** — verified against the file). **BRICK 2.0 appears only for glaciers**: in this
> comparison it is the legacy glacier-only arm, absent elsewhere by scope, not by omission.

> ⚠ **FACTS is seven AR6 workflows, not one number.** The FACTS column is the **median of its 6
> model-class workflows** with **min–max across them** in brackets — a between-workflow spread,
> not a probabilistic band. **`wf4` is structured expert judgement** (bamber19 in both ice
> sheets) and is reported separately, per `benchmark/comparator_classes.csv`.

### Total GMSL — median [band]

| scenario | horizon | Ladrillo L14 | FACTS (6 wf) | FACTS wf4 (SEJ) | MAGICC-SLR |
|---|---|---|---|---|---|
...
**What the table says.** Ladrillo tracks **MAGICC almost exactly at ssp126** (35.1 vs 35.6 at
2100; 45.8 vs 45.9 at 2150; 67.1 vs 66.5 at 2300) while sitting **below FACTS** at every ssp126
cell. At **ssp585 it agrees with MAGICC at 2100** (94.7 vs 97.8, both above FACTS' 78.0) and then
**diverges strongly by 2300 — 514 vs 1016 cm, a factor of ~2.** The ssp245 2300 ordering reverses
(Ladrillo 219 above MAGICC 187). Ladrillo is the *low* member at low forcing and short horizons
and the *low* member again at ssp585/2300; it is not uniformly high or low.
