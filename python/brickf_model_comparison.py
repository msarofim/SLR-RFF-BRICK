#!/usr/bin/env python3
"""
brickf_model_comparison.py — BRICK-F* against FACTS, MAGICC-SLR, and BRICK 2.0.
...
Sources
  BRICK-F*    outputs/ssps_components_2300_extC.csv
...
BAND CAVEAT, stated in every table: BRICK-F* and BRICK 2.0 run on MEAN climate
...
Writes outputs/brickf_model_comparison.csv
"""
import os
...
OUT = os.path.join(REPO, "outputs/brickf_model_comparison.csv")
...
def load_brickf():
    df = pd.read_csv(BRICKF_CSV)
    df["scenario"] = df.ssp.map({v: k for k, v in LABEL.items()})
    return _rows(df, "BRICK-F*", module="extC")
...
def main():
    df = pd.concat([load_brickf(), load_brick20(), load_magicc(), load_facts()],
                   ignore_index=True)
    ...
    print("BRICK-F* vs FACTS / MAGICC-SLR / BRICK 2.0 — cm, rel. 1995-2014 "
