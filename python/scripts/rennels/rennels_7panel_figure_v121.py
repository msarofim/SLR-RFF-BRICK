...
# Posterior variant is env-overridable so one script renders both the pre-PR#93
# (zenodo-v3, default) and post-PR#93 versions:
#   default:                   reads brick_full_<arm>_v121.csv         -> *_v121 outputs
#   RENNELS_VARIANT=_v121_post93 reads brick_full_<arm>_v121_post93.csv -> *_v121_post93 outputs
VARIANT     = os.environ.get("RENNELS_VARIANT", "_v121")
BRICK_LABEL = os.environ.get(
    "RENNELS_BRICK_LABEL", "MimiBRICK v1.2.1 (zenodo-v3 / pre-PR#93 posterior, Wong 2026)")
...
def load(arm):
    df = pd.read_csv(OUT_DIR / f"brick_full_{arm}{VARIANT}.csv").sort_values(KEYS).reset_index(drop=True)
    return df
...
def main():
    ...
    MODES = {
        "per_gtc": dict(
            ...
            fname=f"slr_7panel_ssp245{VARIANT}", sci=False,
        ...
        "abs1em4": dict(
            ...
            fname=f"slr_7panel_ssp245_abs1em4{VARIANT}", sci=True,
    ...
    summ_path = OUT_DIR / f"rennels_pulse_response_summary{VARIANT}.csv"
