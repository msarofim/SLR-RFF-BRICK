...
BRICK_RUNS = [
    ("sneasy_brick",
     "SNEASY-BRICK (SNEASY MAP GMST + OHC)",
     "#1B7837",                                            # dark green
     ROOT / "outputs/brick_obsdriven_sneasyMAP_override_to2024_v121.csv"),
    ("fair_fair",
     "FaIR-BRICK (FaIR-mean GMST + FaIR-mean OHC)",
     "#1F4E79",                                            # navy
     ROOT / "outputs/brick_obsdriven_fair_fair_to2024_v121.csv"),
    ("obs_obs",
     "obs-BRICK (IGCC GMST + Zanna+IGCC OHC)",
     "#A6361C",                                            # maroon
     ROOT / "outputs/brick_obsdriven_obs_obs_igcc_to2024_v121.csv"),
]
...
VARIANT   = "_v121"   # MimiBRICK v1.2.1 + post-#93 posterior; reads *_v121 BRICK CSVs, writes *_v121 figure
...
def main() -> None:
    ...
    # GIS keeps its native calibration obs as the black errorbar; Frederikse 2020
    # is overlaid as a separate labeled series on every panel (see loop below).
    ...
    for ax, (prefix, title, (beg, end), obs_col, sig_col, ylabel) in zip(axes, PANEL_SPEC):
        ...
        # Calibration-data obs overlay (errorbar). GIS is EXCLUDED: post-PR#93 BRICK
        # was calibrated to Frederikse 2020 (the orange overlay), NOT the native
        # IMBIE-merged_greenland calib series (sparse, recent), so showing the latter
        # makes a correctly-fit GIS look mismatched. Compare GIS to Frederikse only.
        if obs_col is not None and obs_col in calib.columns and prefix != "gis":
        ...
        # Frederikse 2020 overlay (every mapped component + the GMSL total)
        try:
            fy, fv, fs = load_frederikse_replacement(prefix, beg, end)
            fm = (fy >= HINDCAST_START) & (fy <= HINDCAST_END)
            ax.plot(fy[fm], fv[fm], color="#E8820C", lw=1.4, alpha=0.9,
                    label="Frederikse 2020")
            ax.fill_between(fy[fm], (fv - fs)[fm], (fv + fs)[fm],
                            color="#E8820C", alpha=0.12)
        except KeyError:
            pass
    ...
    fig.suptitle("BRICK hindcast vs obs — three input scenarios "
                 "(MimiBRICK v1.2.1, post-PR#93 posterior)",
    ...
    out_png = OUT_DIR / f"component_overlay_tony_style_extended{VARIANT}.png"
    out_pdf = OUT_DIR / f"component_overlay_tony_style_extended{VARIANT}.pdf"
