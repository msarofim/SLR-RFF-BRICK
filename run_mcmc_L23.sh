#!/bin/bash
## L23 — RECONSTRUCTED 2026-09-02. L21's calibration with the melt-only glacier ratchet replaced by a floored equilibrium + bounded regrowth (a0155bf). Ran 2026-08-31 22:39. CURRENT CHAMPION.
##
## ⚠⚠ THIS SCRIPT DID NOT EXIST WHEN L23 RAN, AND THAT IS WHY IT EXISTS NOW. Three flags were
...
##   * --adcov  : from the run's own outputs/mcmc/seed_diag_L23_seed*.txt
...
## Pinning means this script reproduces L23 whenever it is run. The original command would
...
##
##   bash run_mcmc_L23.sh [n_iter]        # default 2000000
set -e
cd "$(dirname "$0")"
NITER="${1:-2000000}"
TAG="${TAG:-L23}"
