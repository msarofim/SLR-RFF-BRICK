...
# PROVENANCE CORRECTION (2026-08-19). This said adapted_cov_L12tune_seed2026.csv, and
# the shipped canonical L12 was NOT seeded from it. The script never passed $ADCOV to the
# calibrator, which fell through to its own preference list and took
# adapted_cov_L11tune3_seed2026.csv — log_L12_seed2026.txt records exactly that. L12's
# RESULTS are unaffected: L11tune3 is a legitimately tuned 57-row covariance and, at
# NK=57, it is consumed by the `size == NK` as-is branch, which is correct. But the label
# was wrong and the L12tune covariance was never used. Pinned to what actually ran, so
# this script reproduces the canonical L12 rather than a differently-seeded near-twin.
ADCOV=outputs/mcmc/adapted_cov_L11tune3_seed2026.csv
ADCOV_TUNE=outputs/mcmc/adapted_cov_L12tune_seed2026.csv   # produced by L12tune; unused
STARTS=outputs/mcmc/overdispersed_starts.csv
TUNECHAIN=outputs/mcmc/chain_L12tune_seed2026_n1000000.csv

[[ -f "$ADCOV" ]] || { echo "MISSING $ADCOV — this is the L11 tuning covariance the"; \
  echo "shipped L12 actually ran on; it is not regenerated by an L12 run."; exit 1; }
[[ -f "$ADCOV_TUNE" ]] || { echo "MISSING $ADCOV_TUNE — run the L12tune tuning run first"; \
  echo "(it is what STARTS are built from, even though its covariance went unused):"; \
  echo "  julia --project=julia_v2 julia/calibrate_mcmc_ext.jl 1000000 2026 --tag=L12tune --gis-ordered"; exit 1; }
# NOT passing --adcov here is DELIBERATE: this script reproduces the shipped canonical
# L12, and the shipped run took its seed from the preference list. Adding --adcov would
# be correct going forward but would no longer reproduce 45.53 cm exactly.
