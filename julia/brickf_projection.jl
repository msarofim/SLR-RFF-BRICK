## ============================================================================
## brickf_projection.jl — the BRICK-F* projection kernel (extC posterior)
##
## ONE place that knows how to push a draw of the BRICK-F* posterior through
...
##   is asserted by julia/test_brickf_projection.jl.
##
...
##   bf = brickf_setup(ssp="ssp245", y0=1850, y1=2300)     # forcing + drivers
##   post = brickf_posterior()                             # 10 000 x 52
##   for r in eachrow(post)
##       brickf_run_draw!(bf, r)                           # apply + run(m)
##       total = brickf_series(bf, :total)                 # cm, rel. baseline
##   end
## ============================================================================
...
const BRICKF_REPO = abspath(joinpath(@__DIR__, ".."))
const BRICKF_OBS  = joinpath(BRICKF_REPO, "data/observations")

"""Canonical BRICK-F* posterior subsample (extC, accepted 2026-08-10, commit 205ccbf)."""
const BRICKF_POSTERIOR_CSV =
    joinpath(BRICKF_REPO, "data/MimiBRICK/parameters_subsample_brick_mengel_extC.csv")
const BRICKF_BLOCK_CONSTANTS_CSV = joinpath(BRICKF_REPO, "outputs/extc_block_constants.csv")
const BRICKF_BLOCK_DRIVERS_CSV   = joinpath(BRICKF_OBS, "t_glac_blocks.csv")
"""Medoid row supplying the params the extC posterior does NOT sample (e.g. ais_sea_level₀)."""
const BRICKF_MEDOID_CSV = joinpath(BRICKF_REPO, "outputs/recalib_central_row.csv")

const BRICKF_BLOCKS = ("R19", "SLOWP", "FAST")
"""nu basis: sampled-amp calibration fits (b, T_off) in the obsfit frame — keep consumers aligned."""
const BRICKF_NU_BASIS = "obsfit"
"""Baseline period for reported sea level (AR6 / FACTS-comparable)."""
const BRICKF_REF = (1995, 2014)
"""Rebaseline window for the glacier temperature drivers (the frame contract)."""
const BRICKF_DRIVER_BASE = (1850, 1900)
"""Preserved T_ant(GMST=0) anchor of the DAIS temperature map (calibrate_mcmc_ext.jl A6)."""
const BRICKF_AIS_TANT0 = -15.42 / 0.8365
...
const BRICKF_PHYSICAL_COLS = [p[1] for p in BRICKF_PHYSICAL_PARAMS]
"""Per-block glacier columns applied straight through (kappa is log10 — see apply)."""
const BRICKF_GLACIER_COLS =
    vcat([["gic_a_$b", "gic_b_$b", "gic_T_off_$b"] for b in BRICKF_BLOCKS]...)
"""Posterior columns consumed by `brickf_run_draw!` but not assigned 1:1 to a parameter."""
const BRICKF_DERIVED_COLS = vcat(
    ["ais_runoff_Ton",          # with ais_c -> ais_runoffline_snowheight₀
     "ais_gmst_amp"],           # -> the anchor-preserving DAIS temperature map
    ["gic_log10_kappa_$b" for b in BRICKF_BLOCKS],   # -> gic_kappa_b = 10^theta
    ["gic_amp_$b" for b in BRICKF_BLOCKS])           # -> per-block driver splice
"""Every posterior column this kernel reads."""
const BRICKF_USED_COLS =
    vcat(BRICKF_PHYSICAL_COLS, BRICKF_GLACIER_COLS, BRICKF_DERIVED_COLS)

const _GLACIER_SYMS = Dict(nm => Symbol(nm) for nm in BRICKF_GLACIER_COLS)

"""
    brickf_posterior(; path=BRICKF_POSTERIOR_CSV, cols=:used, nthin=nothing)

Read the BRICK-F* posterior subsample. `cols=:used` reads only the columns the
...
many draws.
"""
function brickf_posterior(; path::AbstractString=BRICKF_POSTERIOR_CSV,
                          cols::Symbol=:used, nthin::Union{Nothing,Int}=nothing)
    df = cols === :all ? CSV.read(path, DataFrame) :
                         CSV.read(path, DataFrame; select=BRICKF_USED_COLS)
...
struct BrickF
    ...
    gmst_rb::Vector{Float64}                 # GMST rebased to BRICKF_DRIVER_BASE
...
"""
    brickf_setup(; ssp="ssp245", y0=1850, y1=2300, forcing_tag=ssp, ref=BRICKF_REF,
                   gmst=nothing, ohc=nothing, lws=:seeded)

Build the BRICK-F* model on a scenario, attach forcing, and precompute the
...
"""
function brickf_setup(; ssp::String="ssp245", y0::Int=1850, y1::Int=2300,
                      forcing_tag::String=ssp, ref::Tuple{Int,Int}=BRICKF_REF,
    ...
    if gmst === nothing
        gmst = [_yearmap(joinpath(BRICKF_OBS, "fair_mean_gmst_$(forcing_tag).csv"), "gmst_C")[y]
                for y in years]
    end
    if ohc === nothing
        ohc = [_yearmap(joinpath(BRICKF_OBS, "fair_mean_ohc_$(forcing_tag).csv"), "ohc_1e22J")[y]
               for y in years]
    end
    length(gmst) == length(years) || error("brickf_setup: gmst has $(length(gmst)) values, need $(length(years))")
    length(ohc)  == length(years) || error("brickf_setup: ohc has $(length(ohc)) values, need $(length(years))")
    ...
    ibase = findall(y -> BRICKF_DRIVER_BASE[1] <= y <= BRICKF_DRIVER_BASE[2], years)
    gmst_rb = Float64.(gmst) .- mean(Float64.(gmst)[ibase])

    tgb = CSV.read(BRICKF_BLOCK_DRIVERS_CSV, DataFrame)
    last_obs = Int(maximum(tgb.year))
    last_obs <= y1 || error("brickf_setup: observed drivers end $last_obs, past the horizon $y1")
    ...
    bc = CSV.read(BRICKF_BLOCK_CONSTANTS_CSV, DataFrame)
    bcrow(b) = bc[findfirst(==(b), bc.block), :]
    nu = Dict(b => Float64(bcrow(b)["nu_anch_$(BRICKF_NU_BASIS)"]) for b in BRICKF_BLOCKS)

    m = build_brick_nu3(ssp=ssp, y0=y0, y1=y1, lws=lws)
    medoid = CSV.read(BRICKF_MEDOID_CSV, DataFrame)[1, :]
    ...
    gic3_init = (; (Symbol(b) => (a     = Float64(bcrow(b).a0),
                                  b     = Float64(bcrow(b)["b_fit_$(BRICKF_NU_BASIS)"]),
                                  T_off = Float64(bcrow(b)["T_off_fit_$(BRICKF_NU_BASIS)"]),
                                  kappa = Float64(bcrow(b)["kappa_anch_$(BRICKF_NU_BASIS)"]),
                                  nu    = nu[b]) for b in BRICKF_BLOCKS)...)
    update_brick_nu3!(m, medoid, gic3_init; precip_log=true)
    set_forcing!(m, gmst, ohc)

    return BrickF(m, ssp, years, [yi(y) for y in ref[1]:ref[2]],
...
"""
brickf_driver(bf::BrickF, block::AbstractString, amp::Real) =
...
amp), and the two derived AIS quantities:
  ...
  * the DAIS temperature map with the T_ant(GMST=0) anchor preserved:
    ...
    scaling moves.
"""
function brickf_apply_draw!(bf::BrickF, row)
    m = bf.m
    @inbounds for (col, comp, sym) in BRICKF_PHYSICAL_PARAMS
        update_param!(m, comp, sym, Float64(row[col]))
    end
    @inbounds for col in BRICKF_GLACIER_COLS
    ...
    for b in BRICKF_BLOCKS
        ...
        update_param!(m, _GLAC, Symbol("glacier_surface_temperature_$b"),
                      brickf_driver(bf, b, Float64(row["gic_amp_$b"])))
    end
    ...
    update_param!(m, _AIS, :ais_temperature_intercept, -BRICKF_AIS_TANT0 / amp)
    return bf
end

"""`brickf_apply_draw!` followed by `run`."""
brickf_run_draw!(bf::BrickF, row) = (brickf_apply_draw!(bf, row); run(bf.m); bf)

## ---------------------------------------------------------------------------
## Reading results
## ---------------------------------------------------------------------------

"""Components readable with `brickf_series`. `gsic_hind` = SLOWP+FAST (the
hindcast/ledger scope, excluding the R19 seam); `total` is BRICK's summed
`global_sea_level` (glaciers + GIS + AIS + TE + LWS)."""
const BRICKF_COMPONENTS = Dict{Symbol,Tuple{Symbol,Symbol}}(
...
brickf_rebase(bf::BrickF, v) =
    (x = [ismissing(e) ? NaN : 100.0 * Float64(e) for e in v]; x .- mean(x[bf.iref]))

"""
    brickf_series(bf, component; funch=nothing)
...
"""
function brickf_series(bf::BrickF, component::Symbol; funch::Union{Nothing,Real}=nothing)
    haskey(BRICKF_COMPONENTS, component) ||
        error("brickf_series: unknown component :$component (have $(sort(collect(keys(BRICKF_COMPONENTS)))))")
    comp, var = BRICKF_COMPONENTS[component]
    raw = bf.m[comp, var]
    if funch !== nothing
        component in (:glaciers, :gsic_hind, :total) ||
            error("brickf_series: F_unch applies to :glaciers, :gsic_hind or :total, not :$component")
        raw = Float64.(raw) .+ Float64(funch) .* bf.funch_unit
    end
    return brickf_rebase(bf, raw)
end

"""Index of year `y` in `bf.years`."""
brickf_yi(bf::BrickF, y::Int) = findfirst(==(y), bf.years)
