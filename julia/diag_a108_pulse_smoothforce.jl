...
function smoothcol(M,K); n=size(M,1); out=similar(M); h=div(K,2); for j in 1:size(M,2), i in 1:n; lo=max(1,i-h); hi2=min(n,i+h); out[i,j]=sum(@view M[lo:hi2,j])/(hi2-lo+1); end; out; end
...
for dn in ("MAGICC","FaIR")
  ...
  gb0=lw(joinpath(FRC,gbf)); ob0=lw(joinpath(FRC,obf);s=OHCS); gp0=lw(joinpath(FRC,gpf)); op0=lw(joinpath(FRC,opf);s=OHCS)
  KF=31  # centered smoothing of the FORCED base; pulse = smoothed_base + clean (pulse-base) delta
  gb=smoothcol(gb0,KF); ob=smoothcol(ob0,KF); gp=gb .+ (gp0 .- gb0); op=ob .+ (op0 .- ob0)
