# Noise-model specification, stream dependence and target grip (extC)
Vintage: **extC (superseded 2026-08-13; stock-SIMPLE Greenland)**; posterior medians from the subsample; residual = model - target, cm; common window 1900-2023.
...
resid sd / mean band σ: ais 0.17, gsic 1.06, gis 1.84, steric 0.90, dang 0.21. Only **gsic, gis, steric** exceed 0.5, i.e. only there does the residual approach its own observation band; on the rest the likelihood is band-dominated and 'AR(1) vs white' compares two terms that barely enter. Full BIC table in `diag_noise_model_streams_extC.csv`.

## F. Item 4.3 — thermal expansion

extC `thermal_alpha` p50 = **0.1540** kg m^-3 C^-1 (90% 0.1424-0.1647), i.e. **0.1011 cm per 1e22 J**. Physics range 0.1011-0.1348. Observed Zanna+IGCC 0.1043; Zanna+Cheng 0.1133 cm per 1e22 J.
